...
export class KpiAdminComponent implements OnInit, OnDestroy {
  ...
  private monthKey(dt: Date) {
    ...
    return `${dt.getFullYear()}-${mm}`; // YYYY-MM
  }
  ...
  private loadData() {
    this.ordersEndpoint.getAll().subscribe({
      ...
      error: (err) => {
        this.alertService.showMessage(this.t('kpiWidget.AdminTitle'), err?.message ?? this.t('kpiWidget.errors.LoadAdmin'), MessageSeverity.error);
      }
    });
  }
  ...
  showMessage(msg: string): void { this.alertService.showMessage(this.t('kpiWidget.AdminTitle'), msg, MessageSeverity.info); }
  ...
  configure(response: boolean, value?: string) {
    if (response) {
      ...
      setTimeout(() => {
        ...
        this.alertService.showMessage(this.t('kpiWidget.AdminTitle'), this.t('kpiWidget.messages.SettingsSaved').replace('{{value}}', String(value ?? '')), MessageSeverity.success);
      }, 2000);
    } else {
      this.alertService.showMessage(this.t('kpiWidget.AdminTitle'), this.t('kpiWidget.messages.Cancelled'), MessageSeverity.default);
    }
  }
